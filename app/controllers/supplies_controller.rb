class SuppliesController < ApplicationController
  ...
  def find
    @supplies = Supply.where("LOWER(name) LIKE ?", "%" + params[:search_string].downcase + "%")

    @supply_name =params[:search_term]
  ...
  private
    ...
    def supply_params
      params.require(:supply).permit(:name, :art_type, :delivery, :price, :stock)
    end
end
